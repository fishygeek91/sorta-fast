...
const LANE3_DMSY_RACE_LIST: readonly RaceAlgoSlug[] = ["dijkstra", "bmssp", "dmsy"];
...
/**
 * @param token - Candidate slug in parsed {@link RaceUrlState.race}.
 * @returns Whether `token` is a supported lane algorithm.
 */
function isRaceToken(token: string): token is RaceAlgoSlug {
 ...
 * @returns Canonical race lane list. `race=` wins over `lane3=` flags.
 */
function parseRaceList(params: URLSearchParams): readonly RaceAlgoSlug[] {
  ...
  if (params.get("lane3") === "1") {
    return LANE3_DMSY_RACE_LIST;
  }
