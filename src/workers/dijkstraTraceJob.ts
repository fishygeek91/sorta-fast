/**
 * Pure Dijkstra trace job: generate graph, stream trace chunks (issue #8).
 *
...
/** Parameters for a single Dijkstra trace run. */
export type DijkstraTraceSpec = {
  kind: GraphKind;
  n: number;
  seed: number;
  source: number;
...
/**
 * Validate {@link DijkstraTraceSpec} before graph generation.
 *
 * @throws When `n`, `seed`, or `source` are out of range; `kind` is checked by {@link generateGraph}.
 */
function validateSpec(spec: DijkstraTraceSpec): void {
...
/**
 * Generate graph, run Dijkstra, stream completed slabs via drainCompleted,
