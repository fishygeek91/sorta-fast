...
| DMSY-P23 | §3.1 Algorithm 2 | Decrease-key vs browser heap | Binary heap with **lazy re-push**. If `v` already in `K`, replace the incoming tree edge and re-push; do not increase `\|K\|`. | DMSY-P07; Dijkstra primitive; no Fibonacci decrease-key |
| DMSY-P24 | §3.1; Appendix A.1; design §4.2 | How do `cut` events encode `{F_j}`? | One `forest` `cut` per tree-edge assigned to an `F_j` (`cut.tree` = `F_j` index). Replay groups cuts by `tree`. Edgeless singleton `F_j` (`k=1`) emits **no** `cut` (`e` must stay a real CSR id for the #23 unmapper). `grow.tree` is the local-search / `F̄` id (separate namespace). | Issue #24 audit AC; `createTraceUnmapper` rejects `e < 0` |
