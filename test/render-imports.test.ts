...
const FORBIDDEN_IMPORT =
  /(?:import|export)\s+(?:type\s+)?(?:\{[^}]*\}|\*\s+as\s+\w+|\w+)\s+from\s+["'][^"']*(?:dijkstra|bellmanFord|trace|bmssp|dmsy)(?:\.ts)?["']/;
...
function assertNoForbiddenImports(dir: string, label: string): void {
  ...
  for (const file of files) {
    ...
    expect(
      source,
      `${label}/${file} must not import dijkstra, bellmanFord, bmssp, dmsy, or trace.ts`,
