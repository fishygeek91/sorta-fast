...
import { runDijkstraTraceJob, type DijkstraTraceSpec } from "../src/workers/dijkstraTraceJob.ts";

/** Collected output from one {@link runDijkstraTraceJob} invocation. */
type JobResult = {
  graph: Graph;
  chunks: TraceChunk[];
  onGraphCalls: number;
...
const SMALL_MAZE_SPEC: DijkstraTraceSpec = {
  kind: "maze",
  n: 40,
  seed: 42,
  source: 0,
...
function sumChunkTallies(chunks: readonly TraceChunk[]): {
  ...
  return { work, relaxations, heapOps };
}
...
describe("runDijkstraTraceJob small maze", () => {
  it("calls onGraph once, onChunk at least once, and end seek has work > 0", () => {
...
describe("runDijkstraTraceJob streaming chunks", () => {
  it("drainCompleted emits multiple onChunk calls before the job returns", () => {
    let onChunkCalls = 0;
    const spec: DijkstraTraceSpec = { ...SMALL_MAZE_SPEC, chunkCapacity: 8 };
...
describe("runDijkstraTraceJob determinism", () => {
...
describe("runDijkstraTraceJob validation", () => {
