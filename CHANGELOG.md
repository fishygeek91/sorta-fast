...
- Open Graph and Twitter Card meta in `index.html` plus `public/og-card.png`; headless contract tests in `test/og-meta.test.ts`, issue #17 visual-token CSS scans in `test/race-css.test.ts`, and post-build `og-card.png` / `og:image` checks in `test/build-workers.ts` (#17).
